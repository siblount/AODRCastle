...
public class GameVictoryScript : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (state == 3)
        {
            ...
            if(sum<4){
                wordstext.text = "Sadness hangs over the exit party. More than half of you haven't came back from the island. This doesn't bode well at all and an even your rescuers are uneasy.";
            }else{
        ...
        if(state == 4){
            ...
            wordstext.text = "You left before the dawn, fought all morning and afternoon, and feast all evening. The music is good and the food is nice. Your reward is even nicer. A hefty sack of gold coins you have trouble carrying. It's going to be hard to spend it in one place, but still doable.";
        }
        if(state == 5){
    ...
    if(state == 10){
        image.sprite = spriteList[8];
            if(Thief.Status == QuestStatus.Success){
                ...
                wordstext.text = "The rogue survived, and is at the party. She is inside the box, even now. She's been smuggling a lot of food down there.";
            }
        }
        if(state == 11){
            image.sprite = spriteList[9];
        ...
        if(state == 13){
            ...
            if(Paladin.Status == QuestStatus.Success){
                ...
                wordstext.text = "The paladin... is at the party? You're dumbfounded at his audacity and approach him. How did he even get here without the boat and without seeing anyone on your team? He sees you and throws down a smoke bomb, vanishing instantly.. WHAT?.";
            }
        }
        if(state == 14){
            image.sprite = spriteList[12];  
        ...
        if(state == 16){
            image.sprite = spriteList[13];
            wordstext.text = "You awake at the same time that you left last night. It's time to go. Thankfully, you were able to go to the local jewlers place and exchange your money for a collection of much easier to handle gems. This'll serve you nicely.";
        }
        if(state == 17){
            image.sprite = spriteList[14];
