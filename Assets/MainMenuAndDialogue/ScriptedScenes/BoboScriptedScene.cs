...
public class BoboScriptedScene : MonoBehaviour
{
    ...
    public void YouPressedButton()
    {
        ...
        if (state == 4)
        {
            if (MarshallQuest.Status != QuestStatus.Success) state = 100;
        ...
        if (state == 9)
        {
            PaladinQuest.Status = QuestStatus.Success;
            PaladinQuest.State = 1;
        ...
        if (state == 100)
        {
            ...
            PaladinQuest.Status = QuestStatus.Failed;
            PaladinQuest.State = 2;
